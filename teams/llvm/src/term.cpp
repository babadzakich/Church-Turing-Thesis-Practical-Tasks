...
std::shared_ptr<Term> Term::clone(std::shared_ptr<Term> t) {
  if (!t)
    throw std::runtime_error("clone: null term");
  switch (t->kind) {
  case TermKind::Var: {
...
static std::string term_to_str(const Term *t, bool paren_abs, bool paren_app) {
  ...
  case TermKind::Abs: {
    std::string s =
        "λ" + t->var + "." + term_to_str(t->body.get(), false, false);
...
std::string Term::to_string() const { return term_to_str(this, false, false); }

// --- α-эквивалентность ---
// Сравниваем де Брёйновские индексы неявно через глубину связывания.

static bool alpha_eq(const Term *a, const Term *b,
                     std::vector<std::string> &env_a,
                     std::vector<std::string> &env_b) {
  if (a->kind != b->kind)
    return false;
  ...
  case TermKind::Var: {
    {
      ...
      if (ia == -1 && ib == -1)
        return a->var == b->var; // оба свободные
