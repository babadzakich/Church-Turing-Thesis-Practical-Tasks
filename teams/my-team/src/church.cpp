...
std::shared_ptr<Term> church_succ() {
    // λn.λf.λx. f (n f x)
    auto n_f_x = Term::make_app(
                     Term::make_app(Term::make_var("n"), Term::make_var("f")),
                     Term::make_var("x"));
...
std::shared_ptr<Term> church_add() {
    ...
    auto nfx = Term::make_app(
                   Term::make_app(Term::make_var("n"), Term::make_var("f")),
                   Term::make_var("x"));
    auto body = Term::make_app(
                    Term::make_app(Term::make_var("m"), Term::make_var("f")),
                    nfx);
    return Term::make_abs("m", Term::make_abs("n",
               Term::make_abs("f", Term::make_abs("x", body))));
}
...
std::shared_ptr<Term> church_pred() {
    ...
    auto body = Term::make_app(
                    Term::make_app(
                        Term::make_app(Term::make_var("n"), pair_step),
                        const_x),
                    id);
    return Term::make_abs("n", Term::make_abs("f", Term::make_abs("x", body)));
}
...
std::shared_ptr<Term> church_is_zero() {
    ...
    auto false_val = Term::make_abs("a", Term::make_abs("b", Term::make_var("b")));
    auto const_false = Term::make_abs("x", false_val);
    auto true_val = Term::make_abs("a", Term::make_abs("b", Term::make_var("a")));
    auto body = Term::make_app(
                    Term::make_app(Term::make_var("n"), const_false),
                    true_val);
...
std::shared_ptr<Term> church_if() {
    // λp.λa.λb. p a b
    auto body = Term::make_app(
                    Term::make_app(Term::make_var("p"), Term::make_var("a")),
                    Term::make_var("b"));
