...
struct Parser {
    ...
    std::string parse_var() {
        ...
        if (pos >= src.size() || !std::isalpha((unsigned char)src[pos]))
            throw std::runtime_error("expected variable name at pos " + std::to_string(pos));
        std::string name;
        while (pos < src.size() &&
    ...
    std::shared_ptr<Term> parse_app() {
        ...
        while (true) {
            ...
            if (c == '\\' ||
                ((unsigned char)c == 0xCE && pos+1 < src.size() &&
                 (unsigned char)src[pos+1] == 0xBB))
            {
...
std::shared_ptr<Term> parse(const std::string& input) {
    ...
    p.skip_ws();
    if (p.pos != input.size())
        throw std::runtime_error("unexpected token at pos " + std::to_string(p.pos));
    return t;
}
